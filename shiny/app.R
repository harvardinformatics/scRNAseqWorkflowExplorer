    ...
    tabPanel(
      ...
      sidebarLayout(
        sidebarPanel(
          width = 4,
          tags$div(
            class = "config-workflow-note",
            tags$p(
              "This shiny app is designed to take inputs generated by our Snakemake workflow that generates Seurat objects produced by different pre-processing workflows found ",
              tags$a("here", href = "https://github.com/harvardinformatics/scRNAseq-preprocessing", target = "_blank"),
              ", and our workflow that evaluates cell clustering robustness through repeated downsampling found ",
              tags$a("here", href = "https://github.com/harvardinformatics/scRNAseqClusterDownsample", target = "_blank"),
              ". In particular, the former adds custom metadata annotations for various QC metrics. If you supply rds files from your own workflow, some tabs will not work if the relevant metadata are missing. If that happens, consult the R scripts run by the Snakemake workflows so that you can adapt them to generate the relevant metadata entries."
            )
          ),
          checkboxInput(
            "workflow_generated_data",
            "Data provided generated by the Snakemake workflows described above",
            value = TRUE
          ),
...
server <- function(input, output, session) {
  ...
  capture_condition_message <- function(expr) {
    tryCatch(
      ...
      shiny.silent.error = function(e) list(result = NULL, error = conditionMessage(e)),
      validation = function(e) list(result = NULL, error = conditionMessage(e)),
      error = function(e) list(result = NULL, error = conditionMessage(e))
    )
  }

  render_tab_error_box <- function(message) {
    ...
    tags$div(
      class = "tab-error-box",
      tags$strong("Full error message"),
      tags$pre(message)
    )
  }
  ...
  output$gene_symbol_datalist <- renderUI({
    methods_result <- all_method_data_safe()

    if (!workflows_confirmed() && !is.null(methods_result$error)) {
      return(NULL)
    }

    methods <- if (workflows_confirmed()) all_method_data() else methods_result$result
  ...
  observe({
    ...
    if (!workflows_confirmed() && !is.null(methods_result$error)) {
      updateSelectInput(session, "heatmap_sort_method", choices = c())
      return()
    }
  ...
  heatmap_error_message <- reactive({
    if (workflows_confirmed() || !identical(input$analysis_tabs, "Gene heatmap") || !nzchar(input$heatmap_gene)) {
      return(NULL)
    }
  ...
  jaccard_error_message <- reactive({
    if (workflows_confirmed() || !identical(input$analysis_tabs, "Cluster similarity heatmap")) {
      return(NULL)
    }
  ...
  output$stability_error <- renderUI({
    render_tab_error_box(stability_error_message())
  })

  output$heatmap_error <- renderUI({
    render_tab_error_box(heatmap_error_message())
  })
  ...
  output$upset_error <- renderUI({
    render_tab_error_box(upset_error_message())
  })

  output$stability_plot <- renderPlot({
    req(is.null(stability_error_message()))
    dat <- loaded_data()
    labels <- stability_plot_labels()
  ...
  output$expression_heatmap <- renderPlot({
    req(is.null(heatmap_error_message()))
    heatmap_obj <- heatmap_data()
    plot_data <- heatmap_obj$plot_data
    validate(
      need(nrow(plot_data) > 0, "No heatmap data available for the selected gene."),
  ...
  output$jaccard_heatmap <- renderPlot({
    req(is.null(jaccard_error_message()))
    dat <- loaded_jaccard_data()
    label_map <- method_labels()
    label1 <- resolve_method_label(label_map, input$jaccard_method1)
    label2 <- resolve_method_label(label_map, input$jaccard_method2)
  ...
  output$status <- renderText({
    if (!workflows_confirmed() && !is.null(stability_error_message())) {
  ...
  output$heatmap_status <- renderText({
    if (!workflows_confirmed() && !is.null(heatmap_error_message())) {
  ...
  output$jaccard_status <- renderText({
    if (!workflows_confirmed() && !is.null(jaccard_error_message())) {
      return(paste0("Full error message:\n", jaccard_error_message()))
    }
  ...
  output$silhouette_stability_status <- renderText({
    if (!workflows_confirmed() && !is.null(silhouette_stability_error_message())) {
      return(paste0("Full error message:\n", silhouette_stability_error_message()))
    }
  ...
  output$upset_status <- renderText({
    if (!workflows_confirmed() && !is.null(upset_error_message())) {
      return(paste0("Full error message:\n", upset_error_message()))
    }
